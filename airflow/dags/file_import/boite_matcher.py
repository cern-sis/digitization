...
from common.storage_connection import StorageProvider
...
class BoiteS3Matcher:
    ...
    def execute(self) -> tuple[dict[str, list[dict]], list[dict]]:
        """Executes the matching logic and returns data purely in memory."""
        results_map = {}
        all_mismatches = []

        logger.info(f"🚀 Starting matching execution for files in: {self.data_path}")

        for box_file in os.listdir(self.data_path):
            if box_file.lower().endswith(".xlsx") and not box_file.startswith("~"):
                logger.info(f"📂 Processing Box file: {box_file}")
