...
class XMLExporter:
    """Handles XML generation and file persistence logic using temporary file storage."""
    ...
    def _build_record_element(self, root: ET.Element, record: dict) -> None:
        ...
        for key, value in record.items():
            if key.endswith("_url") and value:
                ...
                df = ET.SubElement(
                    record_node, "datafield", tag="FFT", ind1=" ", ind2=" "
                )
    ...
    def _save_to_disk(self, root: ET.Element, filename: str) -> str:
        """Converts element tree to an XML file."""
    ...
    def generate_single(self, records: list[dict], filename: str) -> str | None:
        """Generates an XML file for a single Boite file."""
        root = ET.Element("collection")
        valid_records_count = 0

        for rec in records:
            has_valid_url = any(
                key.endswith("_url") and val for key, val in rec.items()
            )
        ...
        if valid_records_count == 0:
            logger.warning(
                f"⚠️ {filename} Skipped: No valid files with presigned URLs found."
            )
            return None
    ...
    def generate_batch(self, results_map: dict[str, list[dict]]) -> dict:
        ...
        logger.info(
            f"✅ XML Generation complete. Total valid files generated: {len(output_report['files'])}"
        )
